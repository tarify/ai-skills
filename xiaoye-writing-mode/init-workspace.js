...
async function initWorkspace(workspacePath = process.cwd()) {
  try {
    console.log(`[XiaoYe] 检查工作区配置: ${workspacePath}`);
    ...
    if (missingFiles.length === 0) {
      console.log(`[XiaoYe] 配置完整，无需初始化`);
      return;
    }
    
    console.log(`[XiaoYe] 发现缺失文件: ${missingFiles.length}个`);
    ...
    for (const file of missingFiles) {
      ...
      try {
        ...
        console.log(`[XiaoYe] 创建文件: ${file}`);
      } catch (err) {
        console.log(`[XiaoYe] 警告: 无法从 scaffold 复制 ${file}: ${err.message}`);
        // 这里可以添加创建默认文件的逻辑
      }
    }
    
    console.log(`[XiaoYe] 工作区初始化完成 ✅`);
    
  } catch (error) {
    console.error(`[XiaoYe] 初始化失败: ${error.message}`);
    throw error;
  }
}
